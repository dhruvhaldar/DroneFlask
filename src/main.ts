  ...
  <div class="workspace">
    ...
    <div class="flight-column">
      <section class="viewport" id="flight-view" tabindex="0" aria-label="Flight simulator. Press F to take off. WASD to move, Space to ascend, Shift to descend.">
        ...
        <div class="minimap"><div class="map-header"><b>${icon("map")} AIRFIELD</b><span>N ↑</span></div><svg viewBox="0 0 170 130" aria-label="Live map showing drone and training gates"><defs><pattern id="mapgrid" width="17" height="17" patternUnits="userSpaceOnUse"><path d="M 17 0 L 0 0 0 17" fill="none" stroke="#8fa99a" stroke-width=".4"/></pattern></defs><rect width="170" height="130" fill="#c1d0c0"/><rect width="170" height="130" fill="url(#mapgrid)"/><path d="M55 0V130" stroke="#8b9d8e" stroke-width="13"/><path d="M55 0V130" stroke="#d5ddce" stroke-width="1" stroke-dasharray="4 4"/><path d="m55 110 0-15 0-18 10-17 14-15 16-18 10-17" fill="none" stroke="#f27748" stroke-width="1.5" stroke-dasharray="3 3"/><g fill="#899e8c"><rect x="28" y="68" width="12" height="15"/><rect x="19" y="43" width="14" height="12"/><rect x="77" y="73" width="10" height="12"/><rect x="96" y="50" width="12" height="13"/></g><g id="map-gates"></g><circle cx="55" cy="110" r="4" fill="white"/><g id="map-drone" transform="translate(55 110)"><circle r="8" fill="#f17848" fill-opacity=".18"/><path d="m0-6 4 10-4-2-4 2Z" fill="#e96d3f" stroke="white" stroke-width="1"/></g></svg><div class="map-footer"><span id="map-coords">0.0, 0.0</span><span>250 m</span></div></div>
        <div class="viewport-bottom"><div class="camera-switch" role="group" aria-label="Camera mode"><button class="active" data-camera="Chase">${icon("video")} Chase</button><button data-camera="FPV">${icon("scan-eye")} FPV</button><button data-camera="Orbit">${icon("orbit")} Orbit</button></div><span class="view-hint" id="view-hint">Scroll to zoom <span>·</span> C to switch camera</span><button class="view-button pause" id="pause" aria-label="Pause simulation">${icon("pause")}</button></div>
      ...
      <section class="mission-card"><div class="mission-symbol">${icon("flag")}</div><div class="mission-copy"><span>GET YOUR WINGS</span><h3>A good flight starts with a little curiosity.</h3><p>Take off, find the orange gates, and explore the airfield at your own pace.</p></div><div class="mission-progress"><div><b id="gate-count">0 / 6</b><span>gates cleared</span></div><div class="progress-track"><i id="gate-progress"></i></div></div></section>
    </div>
  </div>
...
function reset() { flightNumber++; state = initialFlight(); keys.clear(); world?.clearTrail(); lastSample = -1; toast("New flight. The airfield is yours."); refresh(); }
function launch() { if (!world) return; if (state.armed) { state.landing = true; state.returning = false; state.message = "Landing. Hold position for touchdown."; } else takeoff(state); refresh(); }
...
function refresh() {
  ...
  $("map-coords").textContent = `${state.x.toFixed(1)}, ${(-state.z).toFixed(1)}`;
  document.getElementById("map-drone")!.setAttribute("transform", `translate(${Math.max(7, Math.min(163, 55 + state.x * .5))} ${Math.max(7, Math.min(123, 110 + state.z * .5))}) rotate(${heading})`);
  ...
  document.getElementById("map-gates")!.innerHTML = gates.map((g, i) => `<circle cx="${55 + g.x * .5}" cy="${110 + g.z * .5}" r="2.7" fill="${i < state.gates ? "#4e9974" : i === state.gates ? "#f17848" : "#fff"}"/>`).join("");
}
try {
  world = new FlightWorld($("world"));
  world.renderer.setAnimationLoop((now: number) => {
    const dt = Math.min((now - lastFrame) / 1000, .04); lastFrame = now;
    const dialogOpen = !!document.querySelector("dialog[open]");
    if (!dialogOpen) stepFlight(state, keys, dt, mode, wind);
