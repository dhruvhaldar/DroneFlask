...
## 2025-03-08 - ARIA Disclosure Triggers for Inline Confirmations
**Learning:** When a button dynamically spawns an inline warning or confirmation block (acting as a disclosure trigger), screen readers need explicit semantic context to understand the relationship between the button and the newly revealed content. Without these attributes, users may not realize the button controls an expanding section or what content it expands.
**Action:** Always add `aria-expanded` and `aria-controls` attributes to buttons that dynamically spawn inline warnings or confirmations to provide screen readers with explicit semantic context.
