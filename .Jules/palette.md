...
## 2025-03-05 - Keyboard Escape and Focus Management in Inline Dialogs
**Learning:** Attaching an Escape `onKeyDown` handler directly to the trigger button of an inline confirmation dialog creates a trap. If the user tabs into the dialog (e.g., to a Cancel button), the Escape key no longer works because focus has moved. Furthermore, unmounting the active dialog element without manually returning focus causes the browser focus to fall back to the `<body>`, losing the user's place in the interface.
**Action:** Always place `onKeyDown` Escape handlers on a parent wrapper that contains both the trigger and the dialog. Additionally, when dismissing an inline dialog (either via Escape or an explicit Cancel button), programmatically restore focus to the original trigger element using `document.getElementById('id')?.focus()` to maintain logical keyboard flow.
...
## 2026-09-25 - [Simplify Visual Focus Management via CSS]
**Learning:** Managing visual focus accessibility (like `outline` and `outlineOffset`) inline using React state (`useState`, `onFocus`, `onBlur`) adds unnecessary complexity and can be less reliable than native browser pseudo-classes. When working within strict design systems where custom CSS classes are restricted, appending to existing focus blocks (e.g., `:focus-visible`) in the global stylesheet is a cleaner, more robust way to handle focus accessibility natively.
**Action:** Next time you need to ensure focus accessibility on a component, prioritize appending standard pseudo-class selectors (like `:focus-visible`) to existing CSS rules in the global stylesheet rather than implementing inline focus state management via React hooks.
