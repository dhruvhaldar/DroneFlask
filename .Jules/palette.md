...
## 2026-08-20 - [Inline Confirmation ARIA Controls]
**Learning:** When a button triggers a dynamically rendered inline confirmation or warning block (acting as a disclosure widget), screen readers need explicit context that the button controls the new block's visibility.
**Action:** Always pair conditional inline UI blocks with `aria-expanded={true/false}` and `aria-controls="[id]"` on the trigger button.
