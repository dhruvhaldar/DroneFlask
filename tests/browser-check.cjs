...
(async () => {
 ...
 await page.goto(process.env.DRONEVERSE_URL || 'http://127.0.0.1:5173', {waitUntil:'networkidle'});
 ...
 assert.ok(Number(await page.locator('#speed').textContent()) > 5, 'forward keyboard flight');
